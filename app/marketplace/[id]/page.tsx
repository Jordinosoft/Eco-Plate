...
export default function ListingPage({
  ...
  async function handleConfirm() {
    ...
    if (payMethod === "orange") {
      // Direct pay — needs phone number
      setStep("phone");
      return;
    }

    // Payment link — redirect to Fapshi hosted checkout
    setStep("processing");
    try {
      const res = await fetch("/api/pay", {
        ...
        body: JSON.stringify({
          amount: totalXAF,
          message: `FoodLoop: ${listing!.title}`,
  ...
  async function handleDirectPay() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Direct pay failed");
      setStep("phone");
    }
  }
  ...
  // ── Done (direct pay accepted) ───────────────────────────────────────────────
  if (step === "done") {
    return (
      <div className="min-h-screen bg-[var(--ep-cream)] flex flex-col items-center justify-center gap-6 px-4">
        <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[var(--ep-green-light)]">
          <span className="text-3xl">✅</span>
        </div>
        <div className="text-center">
          <h2 className="text-xl font-bold text-gray-900">Payment Initiated!</h2>
          <p className="text-sm text-[var(--ep-neutral)] mt-1">
            A payment prompt has been sent to your Orange Money number. Approve
            it to confirm your rescue.
          </p>
          {transId && (
            <p className="mt-2 text-xs font-mono text-[var(--ep-neutral)]">
              Ref: {transId}
            </p>
          )}
        </div>
        <Link href="/orders/o1" className="btn-cta">
          View My Order →
        </Link>
        <Link href="/marketplace" className="text-sm text-[var(--ep-neutral)] hover:underline">
          Back to Marketplace
        </Link>
      </div>
    );
  ...
  // ── Phone input modal (direct pay) ───────────────────────────────────────────
  if (step === "phone") {
    return (
      <div className="min-h-screen bg-[var(--ep-cream)] flex flex-col items-center justify-center px-4">
        <div className="w-full max-w-sm bg-white rounded-2xl border border-gray-100 p-6 flex flex-col gap-5">
          <div className="text-center">
            <div className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-[var(--ep-orange-light)] mb-3">
              <Phone size={22} className="text-[var(--ep-orange)]" />
            </div>
            <h2 className="text-lg font-bold text-gray-900">Enter Orange Money Number</h2>
            <p className="text-sm text-[var(--ep-neutral)] mt-1">
              A payment prompt of <strong>{formatXAF(totalXAF)}</strong> will be
              sent to your phone.
            </p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[var(--ep-cream)]">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
        <Link
          href="/marketplace"
          className="inline-flex items-center gap-1.5 text-sm text-[var(--ep-neutral)] hover:text-gray-900 mb-5"
        >
        ...
        <div className="grid lg:grid-cols-[1fr_380px] gap-8">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="flex items-center gap-1 text-sm text-[var(--ep-neutral)] mt-1">
                  ...
                  <span className="text-[var(--ep-green-mid)]">0.8 km away</span>
                </div>
              </div>
              <div className="text-right shrink-0">
                ...
                <p className="text-sm text-[var(--ep-neutral)] line-through">
            ...
            <div className="rounded-2xl bg-white border border-gray-100 p-5">
              ...
              <p className="text-sm text-[var(--ep-neutral)] leading-relaxed">
                {listing.description}
              </p>
            ...
            <div className="rounded-2xl bg-white border border-gray-100 p-5">
              <h2 className="font-semibold text-gray-900 mb-2">{listing.merchant}</h2>
              <p className="text-sm text-[var(--ep-neutral)]">{listing.merchantAddress}</p>
              <div className="h-40 rounded-xl mt-3 overflow-hidden bg-gray-100 flex items-center justify-center text-[var(--ep-neutral)] text-sm">
                <MapPin size={16} className="mr-1" /> {listing.merchantAddress}
              </div>
              <button className="btn-outline text-sm w-full justify-center mt-3">
                Open in Maps
              </button>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="rounded-2xl bg-white border border-gray-100 p-5 sticky top-20 flex flex-col gap-4">
              ...
              <div className="flex items-center justify-between font-bold">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-[var(--ep-green-mid)] font-normal">
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  >
                    ...
                    {payMethod === "orange" && (
                      <span className="ml-auto text-[var(--ep-orange)] text-lg">●</span>
                    )}
                  </button>
                  ...
                  >
                    ...
                    {payMethod === "link" && (
                      <span className="ml-auto text-[var(--ep-green)] text-lg">●</span>
                    )}
                  </button>
                </div>
              </div>
              ...
              <button onClick={handleConfirm} className="btn-cta w-full justify-center py-3 text-base">
                {payMethod === "orange" ? "Pay with Orange Money →" : "Go to Secure Checkout →"}
              </button>
              ...
              <div className="flex items-start gap-2 text-xs text-[var(--ep-neutral)] bg-gray-50 rounded-xl p-3">
                ...
                <span>
                  Powered by Fapshi. Your payment is secure and encrypted.{" "}
